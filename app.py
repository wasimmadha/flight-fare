...
from flightFare.entity.flight_predictor import predictor, FlightData
from flask import send_file, abort, render_template

ROOT_DIR = os.getcwd()
LOG_FOLDER_NAME = "logs"
PIPELINE_FOLDER_NAME = "flightFare"
...
def predict():
    if request.method == 'POST':
        ...
        Date_of_Journey = request.json['Date_of_Journey']
        Source = (request.json['Source'])
        Destination = (request.json['Destination'])
        Dep_Time = (request.json['Dep_Time'])
        Arrival_Time = request.json['Arrival_Time']
        Total_Stops = request.json['Total_Stops']
        ...
        flight_predictor = predictor(model_dir=MODEL_DIR)
        flight_value = flight_predictor.predict(X=flight_df)[0]

        # return render_template('predict.html', context=context)
        return jsonify(flight_value)
    return render_template("predict.html", context=context)
